...
A web app that helps Purdue students build dining-court meals that hit specific macro targets (protein, carbs, fat). Pulls live menu data from Purdue's dining API and uses simulated annealing to optimize meal combinations. Optional Gemini AI integration translates plain-English goals like "high protein lunch under 800 calories" into macro targets.
...
4. In the service's **Environment** tab, add `GEMINI_API_KEY` with your key.
5. Deploy. The first build takes ~3 minutes.
...
- **AI**: Google Gemini (`google-genai`) for natural-language goal parsing
